...
# dada2_no_quals.R \
#  --input            input.fq.gz \
#  --errors           DADA2_ErrorRates_noqualErrfun.RData \
#  --bandsize         16 \
...
option_list <- list(
  ...
  make_option(c("-e", "--errors"),           action="store", default=NA,    type='character', help=""),
...
if(is.na(opt$errors)){
  cat("DADA2 error model file is not specified: ....\n", file=stderr())
  stop()
}
...
cat(paste("Pre-computed error model: ", ERROR_MODEL, "\n", sep = ""))
...
errors <- readRDS(file = ERROR_MODEL)
...
qs2::qs_save(object = UC,
  file = "DADA2_UC.qs",
  compress_level = 15L, nthreads = CPUTHREADS)
